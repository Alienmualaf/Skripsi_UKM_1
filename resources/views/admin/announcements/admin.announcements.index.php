...
@section('header', 'Monitoring Pengumuman UKM')

@section('content')
<div style="margin-bottom: 1.5rem;">
    <h3 style="font-size: 1.25rem; font-weight: 800; color: var(--text-primary); margin: 0 0 0.25rem 0;">Monitoring Pengumuman UKM</h3>
    <p style="margin: 0; color: var(--text-secondary); font-size: 0.875rem; line-height: 1.5;">Super Admin mengawasi seluruh pengumuman resmi yang dipublikasikan oleh masing-masing UKM.</p>
</div>
    ...
    <div class="table-wrapper" style="margin-bottom: 0; border: none; padding: 0; box-shadow: none;">
        <table class="table">
            ...
            <tbody>
                ...
                <tr>
                    <td style="font-weight: 600; color: var(--text-primary);">{{ $ann->title }}</td>
                    <td style="font-weight: 600; color: var(--accent-color);">{{ $ann->ukm->name }}</td>
                    <td style="max-width: 250px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ strip_tags($ann->content) }}</td>
                    <td>{{ $ann->creator->name ?? '-' }}</td>
                    <td>{{ \Carbon\Carbon::parse($ann->created_at)->format('d M Y') }}</td>
                    <td>
                        <div style="display: flex; gap: 0.5rem;">
                            <button onclick="editAnnouncement({{ json_encode($ann) }})" class="btn" style="padding: 0.4rem 0.8rem; font-size: 0.8rem; font-weight: 600; background: var(--bg-color); color: var(--accent-color); border: 1px solid var(--border-color);">
                                <i class="ph ph-pencil"></i> Edit
                            </button>
                            <form action="/admin/announcements/{{ $ann->id }}" method="POST" onsubmit="return confirm('Hapus pengumuman ini?')" style="display: inline;">
                                @csrf @method('DELETE')
                ...
                <tr><td colspan="6" class="text-secondary text-center py-8">Tidak ada data pengumuman.</td></tr>
                @endif
            </tbody>
        </table>
    </div>
...
<div id="modal-edit-announcement" class="modal" style="display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.5); align-items: center; justify-content: center; z-index: 1000; backdrop-filter: blur(4px);">
    <div class="card" style="width: 100%; max-width: 550px; margin: 1rem;">
        <h3 style="font-weight: 700; margin-bottom: 1.5rem; color: var(--text-primary);">Edit Pengumuman</h3>
        <form id="edit-form-announcement" method="POST">
            @csrf @method('PUT')
            ...
            <div style="margin-bottom: 1.5rem;">
                <label class="form-label mb-2">Konten</label>
                <textarea name="content" id="edit-content" class="form-control" rows="5" required style="resize: vertical;"></textarea>
            </div>
            <div style="display: flex; gap: 1rem; justify-content: flex-end;">
                <button type="button" onclick="document.getElementById('modal-edit-announcement').style.display='none'" class="btn" style="background: var(--bg-color);">Batal</button>
...
function editAnnouncement(a) {
    document.getElementById('edit-form-announcement').action = '/admin/announcements/' + a.id;
