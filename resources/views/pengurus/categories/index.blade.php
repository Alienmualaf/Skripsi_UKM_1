...
<div style="margin-bottom: 2rem;">
    ...
    <p style="margin: 0; color: var(--text-secondary); font-size: 0.875rem; line-height: 1.5;">Kelola klasifikasi kategori keuangan, jenis persuratan, dan inventaris barang untuk menjaga kerapian data organisasi.</p>
</div>
...
<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 2rem; align-items: start;">
    ...
    <div class="card" style="padding: 1.5rem; display: flex; flex-direction: column; gap: 1.25rem;">
        <h4 style="margin: 0; font-weight: 800; font-size: 1.1rem; display: flex; align-items: center; gap: 0.5rem; color: var(--text-primary);">
            <i class="ph ph-coins" style="color: var(--accent-color); font-size: 1.35rem;"></i> Kategori Keuangan
        </h4>

        <!-- Form Tambah -->
        <form action="{{ route('pengurus.categories.finance.store') }}" method="POST" style="background: #f8fafc; border: 1px solid var(--border-color); padding: 1rem; border-radius: 8px; display: flex; flex-direction: column; gap: 0.75rem;">
            @csrf
            <div>
                ...
                <input type="text" name="name" placeholder="Misal: Uang Kas, Sponsor" required class="form-control" style="width: 100%; padding: 0.45rem 0.75rem; border-radius: 6px; border: 1px solid var(--border-color); font-size: 0.875rem;">
            </div>
            <div>
                <label class="form-label" style="font-size: 0.75rem; font-weight: 800; color: var(--text-secondary); text-transform: uppercase;">Tipe Aliran</label>
                <select name="type" required class="form-control" style="width: 100%; padding: 0.45rem 0.75rem; border-radius: 6px; border: 1px solid var(--border-color); font-size: 0.875rem;">
        ...
        <div style="display: flex; flex-direction: column; gap: 0.5rem;">
            @forelse($financeCategories as $fc)
                <div style="border: 1px solid var(--border-color); padding: 0.75rem; border-radius: 8px; display: flex; justify-content: space-between; align-items: center; background: white;">
                    <div>
                        <div style="font-weight: 700; color: var(--text-primary); font-size: 0.9rem;">{{ $fc->name }}</div>
                        <div style="display: flex; gap: 0.35rem; align-items: center; margin-top: 0.2rem;">
                            @if($fc->type === 'income')
                                <span class="badge" style="background: rgba(16, 185, 129, 0.1); color: var(--success-color); font-size: 0.65rem; font-weight: 800; padding: 0.15rem 0.35rem; border-radius: 4px;">Pemasukan</span>
                            @else
                                <span class="badge" style="background: rgba(239, 68, 68, 0.1); color: var(--danger-color); font-size: 0.65rem; font-weight: 800; padding: 0.15rem 0.35rem; border-radius: 4px;">Pengeluaran</span>
                            @endif
                            <span style="font-size: 0.7rem; color: var(--text-secondary); font-weight: 600;">• {{ $fc->finances_count }} transaksi</span>
                        </div>
                    </div>
                    <div style="display: flex; gap: 0.25rem;">
                        <button onclick="openEditModal('finance', {{ $fc->id }}, '{{ $fc->name }}', '{{ $fc->type }}')" class="btn" style="padding: 0.35rem; background: var(--bg-color); border: 1px solid var(--border-color); color: var(--text-primary); border-radius: 6px;" title="Edit"><i class="ph ph-pencil-simple"></i></button>
                        <form action="{{ route('pengurus.categories.finance.destroy', $fc->id) }}" method="POST" onsubmit="return confirm('Hapus kategori ini?');" style="margin: 0;">
                            @csrf
                            @method('DELETE')
            ...
            @empty
                <div style="text-align: center; font-size: 0.8125rem; color: var(--text-secondary); padding: 1.5rem 0;">Belum ada kategori keuangan.</div>
            @endforelse
        </div>
    </div>
...
<dialog id="editCategoryDialog" style="border: none; border-radius: 12px; box-shadow: 0 10px 25px rgba(0,0,0,0.15); padding: 1.5rem; width: 400px; max-width: 90%;">
    ...
    <form id="editCategoryForm" method="POST">
        ...
        <div id="financeTypeWrapper" style="margin-bottom: 1.25rem; display: none;">
            <label class="form-label" style="font-size: 0.75rem; font-weight: 800; color: var(--text-secondary); text-transform: uppercase;">Tipe Aliran</label>
            <select id="editFinanceType" name="type" class="form-control" style="width: 100%; padding: 0.5rem 0.75rem; border-radius: 8px; border: 1px solid var(--border-color); font-size: 0.875rem; margin-top: 0.25rem;">
                <option value="income">Pemasukan (Income)</option>
                <option value="expense">Pengeluaran (Expense)</option>
            </select>
        </div>
...
<script>
    ...
    const financeWrapper = document.getElementById('financeTypeWrapper');
    const financeSelect = document.getElementById('editFinanceType');

    function openEditModal(group, id, name, type = '') {
        nameInput.value = name;
        
        if (group === 'finance') {
            title.textContent = 'Edit Kategori Keuangan';
            form.action = `/pengurus/categories/finance/${id}`;
            financeWrapper.style.display = 'block';
            financeSelect.value = type;
            ...
            financeWrapper.style.display = 'none';
