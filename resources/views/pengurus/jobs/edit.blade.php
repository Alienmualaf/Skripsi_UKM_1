...
@section('header', 'Edit Penugasan PSUP')

@section('content')
<div style="margin-bottom: 1.5rem;">
    <h3 style="font-size: 1.25rem; font-weight: 800; color: var(--text-primary); margin: 0 0 0.25rem 0;">Perbarui Penugasan Tampil</h3>
    <p style="margin: 0; color: var(--text-secondary); font-size: 0.875rem; line-height: 1.5;">Perbarui rincian konser/acara tampil atau ubah delegasi penyanyi.</p>
</div>
...
<div class="card" style="padding: 2rem;">
    <h4 style="margin: 0 0 1.5rem 0; font-weight: 800; font-size: 1.1rem; display: flex; align-items: center; gap: 0.5rem; color: var(--text-primary);">
        <i class="ph ph-microphone-stage" style="color: var(--accent-color);"></i> Edit Penugasan: {{ $job->title }}
    </h4>

    <form action="{{ route('pengurus.jobs.update', $job->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; margin-bottom: 1.5rem;">
            <div>
                <div class="form-group mb-4">
                    ...
                    <input type="text" name="title" class="form-control" required value="{{ old('title', $job->title) }}" placeholder="Contoh: Gigs Wisuda UP Semester Genap" style="padding: 0.65rem;">
                ...
                <div class="form-group mb-4">
                    ...
                    <textarea name="description" class="form-control" rows="3" placeholder="Contoh: Menyanyikan lagu Hymne Universitas Pancasila dan Bagimu Negeri." style="padding: 0.65rem;">{{ old('description', $job->description) }}</textarea>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1rem;">
                    <div class="form-group">
                        ...
                        <input type="date" name="date" class="form-control" required value="{{ old('date', $job->date) }}" style="padding: 0.65rem;">
                    ...
                    <div class="form-group">
                        ...
                        <input type="time" name="performance_time" class="form-control" required value="{{ old('performance_time', $job->performance_time ? substr($job->performance_time, 0, 5) : '17:00') }}" style="padding: 0.65rem;">
                ...
                <div class="form-group mb-4">
                    ...
                    <input type="text" name="location" class="form-control" required value="{{ old('location', $job->location) }}" placeholder="Contoh: Gedung Serbaguna UP lt. 2" style="padding: 0.65rem;">
                ...
                <div class="form-group mb-4">
                    <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer; font-weight: 700; font-size: 0.85rem; color: var(--text-primary);">
                        <input type="checkbox" name="show_on_landing" value="1" {{ old('show_on_landing', $job->show_on_landing) ? 'checked' : '' }} style="width: auto; margin: 0; cursor: pointer;">
                            ...
                            <div style="display: flex; flex-direction: column; gap: 0.35rem;">
                                @foreach($memberList as $m)
                                    <label style="display: flex; align-items: center; gap: 0.5rem; font-size: 0.825rem; cursor: pointer; color: var(--text-primary);">
                                        <input type="checkbox" name="member_ids[]" value="{{ $m->id }}" {{ in_array($m->id, $selectedMemberIds) ? 'checked' : '' }} style="width: 1rem; height: 1rem;">
