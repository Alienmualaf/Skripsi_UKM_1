    ...
    <form action="{{ route('pengurus.inventories.update', $inventory->id) }}" method="POST">
        ...
        <div class="form-group mb-4">
            ...
            <select name="category" class="form-control" required style="padding: 0.65rem;">
                <option value="Alat Musik" {{ old('category', $inventory->category) === 'Alat Musik' ? 'selected' : '' }}>Alat Musik (Piano, Keyboard, dll)</option>
                <option value="Kostum" {{ old('category', $inventory->category) === 'Kostum' ? 'selected' : '' }}>Kostum (Jas, Kebaya, Seragam, dll)</option>
                <option value="Sound System" {{ old('category', $inventory->category) === 'Sound System' ? 'selected' : '' }}>Sound System / Audio (Mic, Kabel, dll)</option>
                <option value="Perlengkapan Latihan" {{ old('category', $inventory->category) === 'Perlengkapan Latihan' ? 'selected' : '' }}>Perlengkapan Latihan (Stand Partitur, Map, dll)</option>
                <option value="Perlengkapan Acara" {{ old('category', $inventory->category) === 'Perlengkapan Acara' ? 'selected' : '' }}>Perlengkapan Acara (Dekorasi, dll)</option>
            </select>
